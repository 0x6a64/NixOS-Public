#!/usr/bin/env bash
# Compares the current ephemeral root against the old root snapshots
...
perform_diff() {
    ...
    local tmpfile_current tmpfile_old
    tmpfile_current=$(mktemp)
    tmpfile_old=$(mktemp)

    # Intentional early expansion: locals are out of scope when the EXIT trap fires
    ...
    # In the old snapshot but not in the current root
    while IFS= read -r path || [ -n "$path" ]; do
        local current_path="${CURRENT_ROOT}${path}"
...
echo -e "${YELLOW}     See: impermanence.nix and home.nix${NC}"

...
- **External Monitor Control:** DDC/CI brightness control via ddcutil/ddccontrol and a GNOME quick-settings extension
...
├── zen.nix                # Zen Browser config (kept for reference, inactive)
├── disko.nix              # Disk partitioning
├── impermanence.nix       # Impermanence/stateless config
├── flatpak.nix            # Flatpak support (not currently imported)
...
- **Browser:** Firefox with Stylix integration (Zen Browser config kept inactive)
...
**Rebuild System** (or use the `rebuild` zsh alias):

```bash
sudo nixos-rebuild switch --flake ~/Nixos
```

**Update Flake** (or use the `update` zsh alias, which uses `boot` instead of `switch`):
...
- Weekly automatic GC deletes generations older than 30 days (`nix.gc`)
- Daily scheduled store deduplication (`nix.optimise`)
- Boot menu capped at the last 15 generations (lanzaboote inherits `configurationLimit`)

    ...
    home = {
      ...
      persistence."/persist" = {
        ...
        directories = [
          ...
          # KDE Connect paired device state
          ".config/kdeconnect"
        ...
        files = [
          # Shell history
          ".zsh_history"
          # Powerlevel10k instant prompt cache (keeps first terminal after boot fast)
          ".cache/p10k-instant-prompt-${config.home.username}.zsh"
          # Claude Code account state
          ".claude.json"
          # Task Warrior Storage
          ".taskrc"
        ];
      };
      ...
      file = {
        ...
        ".config/mozilla/firefox/default/chrome/userChrome-custom.css".text = ''
          ...
          /* This just increased the spacing between the buttons not the button size
      ...
      # home-manager symlinks for Firefox land before the impermanence bind mount covers
      # ~/.config/mozilla, so they get hidden. Copy the managed files directly into the
      # persist path after every generation switch so Firefox always sees them.
    ...
    programs = {
      ...
      zsh = {
        ...
        shellAliases = {
          ...
          update = "sudo nix flake update && sudo nixos-rebuild boot --flake /home/user/Nixos --option warn-dirty false";
          rebuild = "sudo nixos-rebuild switch --flake /home/user/Nixos --option warn-dirty false";
          rebuildst = "sudo nixos-rebuild switch --flake /home/user/Nixos --show-trace --option warn-dirty false";
